...
export default class ImportedModuleCollector {
  ...
  _createHooks(modules) {
    const shouldIgnore = (modulePath) => {
      return this._isRelativePath(modulePath) || this._isBuiltin(modulePath);
    };
    const addModule = (name, fileInfo) => {
    ...
    return {
      ...
      'ImportDeclaration': (node, fileInfo) => {
        ...
        const moduleName = this._normalizeModulePath(modulePath);
        addModule(moduleName, fileInfo);
      },

      // Handle `var foo = require('foo');`.
  ...
  /**
   * If the module path contains sub modules,
   * remove the subdirectory path.
   * @private
   */
  _normalizeModulePath(modulePath) {
