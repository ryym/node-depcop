...
function makeDepcopFromCLIOptions(values) {
  ...
  const baseConfig = configPath ? require(configPath) : {};
  const specifiedConfig = translateOptions(values);
  const configObject = Object.assign({}, baseConfig, specifiedConfig);
  const disableDefault = Boolean(configPath || values.noDepcoprc);

  return configureDepcop()(configObject, disableDefault);
}
